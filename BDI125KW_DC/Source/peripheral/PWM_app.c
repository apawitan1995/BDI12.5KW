...
volatile int comparepwm6;

volatile int flag_K1;
volatile int flag_K2;
volatile int flag_K3;
volatile int flag_HK1;
volatile int flag_HK2;
volatile int flag_invalid_K2;
volatile int flag_invalid_K3;
...
void PWM_Boost_Init(void)
{
	...
	compare_value=PWMSP001_Calculate_Compare_Value(PWM_2A_2B_PS_API,66.66f);
	PWMSP001_SetCompare(PWM_2A_2B_PS_API,compare_value);
	CCU41->GCSS |= (uint32_t) ((1 << CCU4_GCSS_S0SE_Pos) &CCU4_GCSS_S0SE_Msk);

	compare_value=PWMSP001_Calculate_Compare_Value(PWM_3A_3B_PS_API,33.33f);
