...
/*
volatile uint32_t ACPT1_OFFSET=2065; // 2065
volatile uint32_t ACPT2_OFFSET=2060; // 2059
volatile uint32_t ACPT3_OFFSET=2060;
volatile uint32_t GRPT1_OFFSET=2060;
volatile uint32_t GRPT2_OFFSET=2060;
volatile uint32_t GRPT3_OFFSET=2060;
*/

volatile uint32_t CT1_Result=0;
volatile uint32_t CT2_Result=0;
volatile uint32_t CT3_Result=0;
volatile uint32_t CT4_Result=0;
volatile uint32_t CT5_Result=0;
volatile uint32_t CT6_Result=0;
volatile uint32_t CT7_Result=0;
volatile uint32_t CT8_Result=0;

//volatile uint32_t DCPT1_AIN_Result=0;
volatile uint32_t DCPT2_AIN_Result=0;
volatile uint32_t ACPT1_AIN_Result=0;
volatile uint32_t ACPT2_AIN_Result=0;
//volatile uint32_t ACPT3_AIN_Result=0;
volatile uint32_t GRPT1_AIN_Result=0;
volatile uint32_t GRPT2_AIN_Result=0;
volatile uint32_t GRPT3_AIN_Result=0;

//volatile uint32_t TQ1_AIN_Result=0;
//volatile uint32_t TQ2_AIN_Result=0;
//volatile uint32_t AIS1_AIN_Result=0;
//volatile uint32_t AIS2_AIN_Result=0;
...
void ADC_G0_Scan_Init(void)
{
	...
	//Select G0CH4
	SET_BIT(VADC_G0->ASSEL,VADC_G_ASSEL_CHSEL4_Pos);
	//Select G0CH5
	SET_BIT(VADC_G0->ASSEL,VADC_G_ASSEL_CHSEL5_Pos);
	//Select G0CH7
	SET_BIT(VADC_G0->ASSEL,VADC_G_ASSEL_CHSEL7_Pos);
	...
	VADC_G0->ASCTRL |= (uint32_t)
					...
					(((uint32_t)  0x0B<< VADC_G_ASCTRL_XTSEL_Pos) &VADC_G_ASCTRL_XTSEL_Msk);
	...
	//Clear bit RESTBS to allow store result in source register
	CLR_BIT(VADC_G0->CHCTR[4],VADC_G_CHCTR_RESTBS_Pos);
	//Store result ADC G0CH4 in RESULT 4 Register
	WR_REG(VADC_G0->CHCTR[4],VADC_G_CHCTR_RESREG_Msk,VADC_G_CHCTR_RESREG_Pos,0x04);

	//Clear bit RESTBS to allow store result in source register
	CLR_BIT(VADC_G0->CHCTR[5],VADC_G_CHCTR_RESTBS_Pos);
	//Store result ADC G0CH5 in RESULT 5 Register
	WR_REG(VADC_G0->CHCTR[5],VADC_G_CHCTR_RESREG_Msk,VADC_G_CHCTR_RESREG_Pos,0x05);

	//Clear bit RESTBS to allow store result in source register
	CLR_BIT(VADC_G0->CHCTR[7],VADC_G_CHCTR_RESTBS_Pos);
	//Store result ADC G0CH5 in RESULT 7 Register
	WR_REG(VADC_G0->CHCTR[7],VADC_G_CHCTR_RESREG_Msk,VADC_G_CHCTR_RESREG_Pos,0x07);

	//Configure Result interrupt for RESG0_CH1

//	//Enable service result generation				//DIKOMEN DULU
//	SET_BIT(VADC_G0->RCR[1],VADC_G_RCR_SRGEN_Pos);
//	//Route ADC_G0_RES01 to VADC.G0.SR1				//DIKOMEN DULU
...
void ADC_G0_Init(void)
{
	...
	//G0CH4
	SET_BIT(VADC_G0->CHASS,VADC_G_CHASS_ASSCH4_Pos);
	//G0CH5
	SET_BIT(VADC_G0->CHASS,VADC_G_CHASS_ASSCH5_Pos);
	//G0CH7
