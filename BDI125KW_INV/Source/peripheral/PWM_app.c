...
volatile int comparepwm4 = 3999;
volatile int comparepwm5 = 3999;
volatile int comparepwm6 = 3999;
volatile int comparepwm7 = 3999;


//volatile uint8_t PWM_App_Run_Stat=0;
volatile float CT3_Result_f;
volatile float update_DC_f;
volatile int32_t testpwmfixed = 3749;
volatile int32_t pwmol = 3649;
volatile int32_t testpwmfixedinv = 4;

volatile float testpwm = 0;
volatile float testpwmINV = 0;
volatile float pwmtestinverter1 = 0;
volatile float pwmtestinverter2 = 0;

volatile int32_t EN_CV = 0;
volatile uint32_t lcd_refresh_counter = 16000;
//volatile uint32_t counter_ref = 32000;
volatile uint32_t en_lcd_print = 0;
volatile uint32_t offset_rate = 0;
...
void PWM6A6B_Init(void)
{
	...
	SET_BIT(PWM_6A_6B->CHC,CCU8_CC8_CHC_OCS3_Pos);
	...
	//Init PWM6A P1.10 (CCU81.OUT21) dan PWM6B P1.13 (CCU81.OUT20)
	//Set passive level low P5.2
	CLR_BIT(PWM_6A_6B->PSL,CCU8_CC8_PSL_PSL11_Pos);
	//Set passive level low P5.3
	CLR_BIT(PWM_6A_6B->PSL,CCU8_CC8_PSL_PSL12_Pos);

	//Set output selector non inverted output to CCU81.OUT21
	SET_BIT(PWM_6A_6B->CHC,CCU8_CC8_CHC_OCS2_Pos);
	//Set output selector inverted output to CCU81.OUT20
	SET_BIT(PWM_6A_6B->CHC,CCU8_CC8_CHC_OCS1_Pos);

	//Enable Dead Time for Channel 1
	SET_BIT(PWM_6A_6B->DTC,CCU8_CC8_DTC_DTE1_Pos);

	//Dead Time enabled for non inverted output CCU81.OUT21
	SET_BIT(PWM_6A_6B->DTC,CCU8_CC8_DTC_DCEN1_Pos);
	//Dead Time enabled for inverted output CCU81.OUT20
	SET_BIT(PWM_6A_6B->DTC,CCU8_CC8_DTC_DCEN2_Pos);

	//Set dead time channel 2 value for Rising edge
	WR_REG(PWM_6A_6B->DC1R,CCU8_CC8_DC1R_DT1R_Msk,CCU8_CC8_DC1R_DT1R_Pos,PWM_6A_6B_Handle.kRisingDeadTime);
	//Set dead time channel 2 value for Falling edge
	WR_REG(PWM_6A_6B->DC1R,CCU8_CC8_DC1R_DT1F_Msk,CCU8_CC8_DC1R_DT1F_Pos,PWM_6A_6B_Handle.kFallingDeadTime);

	//Select P1.10 Driver Pad
	WR_REG(PORT1->PDR1,PORT1_PDR1_PD10_Msk,PORT1_PDR1_PD10_Pos,0);
	//Select P1.13 Driver Pad
	WR_REG(PORT1->PDR1,PORT1_PDR1_PD13_Msk,PORT1_PDR1_PD13_Pos,0);
}
...
void PWM_Inverter_Enable(void)
{
	...
	//Enable P1.10 as Push-Pull Output and Connect CCU81.OUT21 to P1.10
	WR_REG(PORT1->IOCR8,PORT1_IOCR8_PC10_Msk,PORT1_IOCR8_PC10_Pos,0x13);
	//Enable P1.13 as Push-Pull Output and Connect CCU81.OUT20 to P1.13
	WR_REG(PORT1->IOCR12,PORT1_IOCR12_PC13_Msk,PORT1_IOCR12_PC13_Pos,0x13);
}
...
void PWM_Inverter_Disable(void)
{
	...
	//Enable P1.10 as Direct I/O Push-Pull Output
	WR_REG(PORT1->IOCR8,PORT1_IOCR8_PC10_Msk,PORT1_IOCR8_PC10_Pos,0x10);
	CLR_BIT(PORT1->OUT,PORT1_OUT_P10_Pos);
	//Enable P1.13 as Direct I/O Push-Pull Output
	WR_REG(PORT1->IOCR12,PORT1_IOCR12_PC13_Msk,PORT1_IOCR12_PC13_Pos,0x10);
	CLR_BIT(PORT1->OUT,PORT1_OUT_P13_Pos);
}
